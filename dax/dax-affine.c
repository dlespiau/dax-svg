 ...
 *
 * This file was taken from librsvg, rsvg-affine.c, LGPLv2+
 ...
 * The manipulated matrices look like
 *      a c e
 *      b d f
 *      0 0 1
 * with the array holding a, b, c, d, e, f
 */
...
/**
 * _dax_affine_invert: Find the inverse of an affine transformation.
 ...
 **/
void
_dax_affine_invert (double       dst[6],
                    const double src[6])
{
    double r_det;
...
/**
 * _dax_affine_flip: Flip an affine transformation horizontally and/or
 * vertically.
 ...
 **/
void
_dax_affine_flip (double       dst_affine[6],
...
/**
 * _dax_affine_multiply: Multiply two affine transformation matrices.
 ...
 **/
void
_dax_affine_multiply (double       dst[6],
                      const double src1[6],
                      const double src2[6])
{
...
/**
 * _dax_affine_identity: Set up the identity matrix.
 * @dst: Where to store the resulting affine transform.
 *
 * Sets up an identity matrix.
 **/
void
_dax_affine_identity (double dst[6])
{
    dst[0] = 1;
    dst[1] = 0;
    dst[2] = 0;
...
/**
 * _dax_affine_scale: Set up a scaling matrix.
 * @dst: Where to store the resulting affine transform.
 * @sx: X scale factor.
 * @sy: Y scale factor.
 *
 ...
 **/
void
_dax_affine_scale (double dst[6],
                   double sx,
                   double sy)
{
    dst[0] = sx;
...
/**
 * _dax_affine_rotate: Set up a rotation affine transform.
 * @dst: Where to store the resulting affine transform.
 ...
 **/
void
_dax_affine_rotate (double dst[6],
                    double theta)
{
    double s, c;
...
/**
 * _dax_affine_shear: Set up a shearing matrix.
 * @dst: Where to store the resulting affine transform.
 ...
 **/
void
_dax_affine_shear (double dst[6],
                   double theta)
{
    double t;
...
/**
 * _dax_affine_translate: Set up a translation matrix.
 * @dst: Where to store the resulting affine transform.
 ...
 **/
void
_dax_affine_translate (double dst[6],
                       double tx,
                       double ty)
{
    dst[0] = 1;
...
/**
 * _dax_affine_expansion: Find the affine's expansion factor.
 ...
 **/
double
_dax_affine_expansion (const double src[6])
{
    return sqrt (fabs (src[0] * src[3] - src[1] * src[2]));
}

/**
 * _dax_affine_rectilinear: Determine whether the affine transformation is
 * rectilinear.
 * @src: The original affine transformation.
 *
 ...
 **/
int
_dax_affine_rectilinear (const double src[6])
{
...
/**
 * _dax_affine_equal: Determine whether two affine transformations are equal.
