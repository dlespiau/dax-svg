...
static void                                                                 \
transform_castet_##enum_type##_string (const GValue *src,                   \
                                       GValue       *dest)                  \
{                                                                           \
    const gchar *name;                                                      \
    gint value;                                                             \
    ...
    if (name)                                                               \
        ...
        g_value_set_static_string (dest,                                    \
                               "<invalid Castet" G_STRINGIFY(EnumType) ">");\
}                                                                           \
                                                                            \
static void                                                                 \
transform_string_castet_##enum_type (const GValue *src,                     \
                                     GValue       *dest)                    \
{                                                                           \
    const gchar *name;                                                      \
    gint value;                                                             \
...
#define INSTALL_TRANSFORM_FUNCS(enum_type, ENUM_TYPE)                       \
    g_value_register_transform_func (CASTET_TYPE_##ENUM_TYPE,               \
                                     G_TYPE_STRING,                         \
                                     transform_castet_##enum_type##_string);\
    g_value_register_transform_func (G_TYPE_STRING,                         \
                                     CASTET_TYPE_##ENUM_TYPE,               \
                                     transform_string_castet_##enum_type);

static enum_value svg_version_enum_values[3] = {
    { CASTET_SVG_VERSION_1_0, "1.0" },
    { CASTET_SVG_VERSION_1_1, "1.1" },
    { CASTET_SVG_VERSION_1_2, "1.2" }
};

DEFINE_TRANSFORM_FUNCS(svg_version, SvgVersion)

static enum_value script_type_enum_values[2] = {
    { CASTET_SCRIPT_TYPE_ECMASCRIPT,   "application/ecmascript"   },
    { CASTET_SCRIPT_TYPE_JAVA_ARCHIVE, "application/java-archive" }
};
